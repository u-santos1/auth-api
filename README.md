...
## 🚀 Funcionalidades e Defesas (AppSec)

Além de ser uma API de autenticação, este projeto implementa defesas ativas contra vulnerabilidades do OWASP Top 10 e ameaças automatizadas (OAT):

- **Registro Seguro e Granular:** Criação de contas com validação rigorosa via DTOs. Possui **Rate Limit Granular** baseado em IP (Caffeine Cache + Bucket4j) restrito a 3 contas/hora por IP para mitigar Account Creation Spam (OAT-019).
- **Proteção contra JWT "Imortais":** Validação profunda no `SecurityFilter` checando o estado transacional do `UserDetails` (`isEnabled`, `isAccountNonLocked`), garantindo que usuários desativados por um admin percam o acesso imediatamente, mesmo com tokens dentro da validade.
- **Defesa contra DoS e OOM:** Rate Limiting global inteligente. Previne bypasses via manipulação de proxy (`X-Forwarded-For`) e evita ataques de exaustão de memória (Memory Leak) utilizando políticas rigorosas de expiração de cache.
- **Criptografia e Validação de Senhas:** Nenhuma senha é salva em texto limpo (BCrypt). Regras forçam no mínimo 8 caracteres alfanuméricos mitigando ataques de força bruta.
- **Controle de Perfis Rigoroso (RBAC):** Separação validada por testes de integração entre perfis (`ADMIN` e `USER`), prevenindo escalada de privilégios genérica.
- **Infraestrutura Resiliente:** Tratamento global de exceções ocultando *stack traces* sensíveis do cliente e versionamento seguro de banco de dados com Flyway e PostgreSQL em containers Docker.